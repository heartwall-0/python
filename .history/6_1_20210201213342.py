...
LastEditTime: 2021-02-01 21:33:42
...
'''
使用get()来获取键值对是否存在，当你使用print()输出一个键时，但是在字典中该键不存在，那么python会报错，
所以使用get()会避免这种错误的出现，如下所示：
'''
alien_3 = {'color': 'green', 'speed': 'slow'}

#todo 当字典中没有键值使用get()时，可以指定后面的消息，用来提醒。避免错误，但是如果不指定，那么返回None。
point_value = alien_3.get('points', 'No point value assigned.') #! 输出: No point value assigned.
print(point_value)